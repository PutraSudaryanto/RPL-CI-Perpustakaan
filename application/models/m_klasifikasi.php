...
class M_klasifikasi extends CI_Model{
    ...
    function get_all($offset = 0, $keywords = "none")
	{
		if($keywords && $keywords != "none") {
			$this->db->like("jenis_klasifikasi", $keywords);
		}
	...
	function combo_data($options = array())
	{
		...
		foreach($result as $row) {
			$options[$row->id_klasifikasi] = $row->jenis_klasifikasi;
		}
	...
	{
		return $this->db->get_where($this->table, array("id_klasifikasi" => $id))->row();
	}
    ...
    function cari($cari){
        ...
        $this->db->or_like("jenis_klasifikasi",$cari);
